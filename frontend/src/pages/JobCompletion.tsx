...
import type { RepairRequest, AssignmentDetail, AssignmentResponse } from '../types/types';
...
const JobCompletion: React.FC = () => {
  ...
  const isFormValid = summary.trim() !== '' && hasSignature;
  ...
  return (
    <Layout title={`ปิดงานซ่อม (#REQ-${request.id.toString().padStart(4, '0')})`} showBackButton showBottomNav={false}>
      <div className={`${styles.container} ${isSubmitting ? styles.submitting : ''}`}>
        ...
        <section className={styles.section}>
          ...
          <div className={styles.uploadArea}>
            <span className="material-symbols-outlined" style={{ fontSize: '36px', color: 'var(--color-primary)' }}>add_a_photo</span>
            <p className={styles.uploadText}>แตะเพื่อถ่ายรูปหรือเลือกรูปภาพ</p>
            <p className={styles.uploadSubtext}>อัปโหลดรูปภาพความละเอียดสูงเพื่อยืนยันการซ่อมแซม</p>
          </div>
        </section>
        ...
        <section className={styles.section}>
          ...
          <div className={styles.inputGroup}>
            <label className={styles.label} htmlFor="summary">รายละเอียดการซ่อมแซม</label>
            <textarea 
              id="summary"
        ...
        <section className={styles.section}>
          ...
          <p className={styles.infoText}>
            <span className="material-symbols-outlined" style={{ fontSize: '16px', marginRight: '4px', verticalAlign: 'text-bottom' }}>info</span>
            ...
            {assignments.map((tech) => (
              <div key={tech.technician_id} className={styles.techCard}>
                <div className={styles.techHeader}>
                  ...
                  <div>
                    <h3 className={styles.techName}>{tech.technician_name || `ช่างเทคนิค ID: ${tech.technician_id}`}</h3>
                    <span className={styles.techRoleBadge}>{tech.is_leader ? 'Leader' : 'Assistant'}</span>
                  </div>
                </div>
                ...
                <div className={styles.starRating}>
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button 
                      ...
                      onClick={() => handleRatingChange(tech.technician_id, star)}
                      disabled={isSubmitting}
                    >
                      <span className="material-symbols-outlined" style={{ fontVariationSettings: (ratings[tech.technician_id]?.rating || 0) >= star ? '"FILL" 1' : '"FILL" 0' }}>star</span>
                    </button>
                  ))}
                </div>
                ...
                <input 
                  ...
                  onChange={(e) => handleCommentChange(tech.technician_id, e.target.value)}
                  disabled={isSubmitting}
                />
              </div>
            ))}
        ...
        <section className={styles.section}>
          ...
          <p className={styles.subLabel}>กรุณาให้ลูกค้าเซ็นชื่อเพื่อยืนยันการรับมอบงาน</p>
          ...
          <div className={styles.signatureActions}>
            <button className={styles.clearBtn} onClick={clearSignature} disabled={isSubmitting || !hasSignature}>ล้างลายเซ็น</button>
            <button className={styles.confirmSigBtn} disabled={isSubmitting || !hasSignature}>ยืนยันลายเซ็น</button>
          </div>
        </section>
        ...
        <div className={styles.submitArea}>
          ...
          {!isFormValid && (
            <p className={styles.errorText}>กรุณากรอกรายละเอียดการซ่อมและเซ็นชื่อให้ครบถ้วน</p>
          )}
        </div>
